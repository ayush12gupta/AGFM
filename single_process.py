...
parser.add_argument('--config', type=str, default="./config/data_config.json", help="Data config file")
...
def main():
    ...
    execute(f'python3 {cwd}/preprocessing/setup_env.py --reference {args.reference} --secondary {args.secondary} --orbit_path {config["Orbit_data"]} --data_pathR {config["SAR_dir"]} --data_pathS {config["SAR_dir"]}')
