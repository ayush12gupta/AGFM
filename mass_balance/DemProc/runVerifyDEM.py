...
def runVerifyDEM(self, zeroDEM=False):
    '''
    ...
    if not zeroDEM:
        ...
        demFilename = self.demzeroFilename

    if demFilename not in ['',None]:
        demimg = isceobj.createDemImage()
        demimg.load(demFilename + '.xml')
        if not os.path.exists(demFilename + '.vrt'):
            demimg.renderVRT()

        if demimg.reference.upper() == 'EGM96':
            wgsdemname  = demFilename + '.wgs84'
