...
def runMergeBursts(self, adjust=1):
    ...
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'los_dem_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'los_dem.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    ...
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'hgt_dem_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'hgt.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'hgt_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'hgt_zero.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'incid_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'incid.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    ...
    if suffix not in ['',None]:
        if self.doInSAR:
            ...
            multilook(os.path.join(mergedir, 'los_dem.rdr'+suffix),
              outname = os.path.join(mergedir, 'los_dem.rdr'),
              alks = self.numberAzimuthLooks, rlks=self.numberRangeLooks)

            multilook(os.path.join(mergedir, 'incid.rdr'+suffix),
              outname = os.path.join(mergedir, 'incid.rdr'),
              alks = self.numberAzimuthLooks, rlks=self.numberRangeLooks)

            multilook(os.path.join(mergedir, 'hgt.rdr'+suffix),
              outname = os.path.join(mergedir, 'hgt.rdr'),
              alks = self.numberAzimuthLooks, rlks=self.numberRangeLooks)

            multilook(os.path.join(mergedir, 'hgt_zero.rdr'+suffix),
              outname = os.path.join(mergedir, 'hgt_zero.rdr'),
              alks = self.numberAzimuthLooks, rlks=self.numberRangeLooks)
