...
def createZeroDEM(self):
    ...
    elevation = np.memmap(outname, dtype=np.float32, mode='w+', shape=(length,width))
    elevation[:,:] = 0
...
def runTopoZeroCPU(self):
    from zerodop.topozero import createTopozero
    from isceobj.Planet.Planet import Planet

    swathList = self._insar.getValidSwathList(self.swaths)

    if not os.path.exists(self.demzeroFilename):
        createZeroDEM(self) # Create zero DEM if not exists

    ####Catalog for logging
    catalog = isceobj.Catalog.createCatalog(self._insar.procDoc.name)

    ####Load in DEM
    demfilename = self.verifyDEM(zeroDEM=True)
    ...
    catalog.printToLog(logger, "runTopoZero")
