...
#        ChipRangeX = ChipRangeX[ChipRangeX < (2**8 - 1)]
        ...
        for i in range(ChipSizeUniX.__len__()-1, -1, -1):
            ...
            if self.IMG.dtype == np.uint8:
                ...
                sys.exit('invalid data type for the image pair which must be unsigned integer 8 or 32-bit float')
            print("Time taken for Fine Search", time.time()-chpt1)
            ...
            chpt2 = time.time()
            print("Total Time taken", chpt2-start_time)
...
def unpacking_column_u(tup):
    ...
    for ii in range(in_shape[0]):
        ...
        if SubPixFlag:
            # call C++
            Dx[ii], Dy[ii], SNR[ii] = np.float32(compute_dx(ChipShape, ChipI.astype('uint8'), RefShape, RefI.astype('uint8'), scale=oversample, interp=INTERP_METHOD))
            # SNR[ii] = 10*np.log10(SNR[ii]**2)
        else:
            # call C++
            Dx[ii], Dy[ii], SNR[ii] = np.float32(compute_dx(ChipShape, ChipI.astype('uint8'), RefShape, RefI.astype('uint8'), scale=1, interp=INTERP_METHOD))
            # SNR[ii] = 10*np.log10(SNR[ii]**2)
            
    return Dx, Dy, 10*np.log10(SNR**2)
...
def unpacking_column_s(tup):
    ...
    for ii in range(in_shape[0]):
        ...
        if SubPixFlag:
            # call C++
            Dx[ii], Dy[ii], SNR[ii] = np.float32(compute_dx(ChipShape, ChipI.ravel(), RefShape, RefI.ravel(), scale=oversample, interp=INTERP_METHOD))
            # SNR[ii] = 10*np.log10(SNR[ii]**2)
        else:
            # call C++
            Dx[ii], Dy[ii], SNR[ii] = np.float32(compute_dx(ChipShape, ChipI.ravel(), RefShape, RefI.ravel(), scale=1, interp=INTERP_METHOD))
            # SNR[ii] = 10*np.log10(SNR[ii]**2)
    ...
    if MultiThread == 0:
        for jj in range(xGrid.shape[1]):
            ...
            for ii in range(xGrid.shape[0]):
                ...
                if SubPixFlag:
                    # call C++
                    Dx1[ii], Dy1[ii], SNR1[ii] = np.float32(compute_dx(ChipShape, ChipI.ravel(), RefShape, RefI.ravel(), scale=oversample, interp=INTERP_METHOD))
#                   # call Python
                else:
                    # call C++
                    Dx1[ii], Dy1[ii], SNR1[ii] = np.float32(compute_dx(ChipShape, ChipI.ravel(), RefShape, RefI.ravel(), scale=1, interp=INTERP_METHOD))
#                   # call Python
