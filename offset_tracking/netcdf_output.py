...
#    var.setncattr('autoRIFT_software_version',version)
    ...
    print(VX.shape, VY.shape, tran)
    nc_outfile.createDimension('x',dimidX)
    nc_outfile.createDimension('y',dimidY)
#    pdb.set_trace()
    print("Arrnge", tran[0],tran[0]+tran[1]*(dimidX-1),tran[1])
    print("Arrnge", tran[3],tran[3]+tran[5]*(dimidY-1),tran[5])
    x = np.arange(tran[0],tran[0]+tran[1]*(dimidX-1),tran[1])
    if x.shape[0]!=dimidX:
        x = np.linspace(x[0], x[-1], dimidX)
    y = np.arange(tran[3],tran[3]+tran[5]*(dimidY-1),tran[5])
    if y.shape[0]!=dimidY:
        y = np.linspace(y[0], y[-1], dimidY)
    # print(x.shape, y.shape, dimidX, dimidY)
#    pdb.set_trace()
    chunk_lines = np.min([np.ceil(8192/dimidY)*128, dimidY])
#    ChunkSize = [dimidX, chunk_lines]
    ChunkSize = [chunk_lines, dimidX]
...
#        vx_error_slow = np.std(temp[(temp > -500)&(temp < 500)])
    ...
    # print(var, "!!!", np.round(np.clip(VX, -32768, 32767)).astype(np.int16).shape)
