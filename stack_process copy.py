...
def stack_offset_tracking(config_path, download_txt, data_path, polarization, mask, save_path, mode):
    ...
    if len(glob.glob('merged/SLC/*/*.slc.full'))==0:
        ...
        if not os.path.exists('./run_files'):
            execute(f'python3 {DIR_PATH}/stack/topsStack/stackSentinel.py -s {data_path} -d {dem[0]} -o {config["Orbit_dir"]} -p {polarization} -e 0.6 --num_proc 64 -b "{config_isce["ROI"][1:-1].replace(",","")}" -t "python3 {DIR_PATH}/stack/topsStack/" -W slc')
#  -a {aux}
...
if __name__=='__main__':
    ...
    stack_offset_tracking(args.config, args.download_txt, args.data_path, args.polarization, args.mask, args.save_path, args.mode)
